...
export default function AnnouncementsClient({ session }: { session: SessionUser }) {
  ...
  const PRI: Record<string,{c:string;b:string;label:string}> = { Normal:{c:'#6B6B8A',b:'#4A4A6A',label:''}, Important:{c:'#FBBF24',b:'#FBBF24',label:'★ '}, Urgent:{c:'#F87171',b:'#EF4444',label:'🚨 '} }
  ...
  return (
    <div>
      <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:20}}>
        <h2 style={{color:'white',fontFamily:"'Space Grotesk',sans-serif",fontSize:20,fontWeight:700}}>Announcements ({items.length})</h2>
        {canPost && <button onClick={()=>setShowCreate(true)} style={{padding:'9px 18px',background:'#E8630A',border:'none',borderRadius:9,color:'white',fontWeight:700,fontSize:13,cursor:'pointer',fontFamily:"'DM Sans',sans-serif"}}>+ Post Announcement</button>}
      </div>
      <div style={{display:'flex',flexDirection:'column',gap:12}}>
        {items.map((a:any) => {
          ...
          const c = a.creator||{}
          return (
            <div key={a.id} style={{background:a.priority==='Urgent'?'rgba(239,68,68,0.04)':a.priority==='Important'?'rgba(251,191,36,0.04)':'#111120',border:'1px solid #1E1E35',borderLeft:`3px solid ${p.b}`,borderRadius:14,padding:'20px 22px'}}>
              <div style={{display:'flex',justifyContent:'space-between',alignItems:'flex-start',marginBottom:10}}>
                <h3 style={{color:'white',fontWeight:700,fontSize:16,fontFamily:"'Space Grotesk',sans-serif"}}>{p.label}{a.title}</h3>
              ...
              <div style={{display:'flex',justifyContent:'space-between',alignItems:'center'}}>
                <div style={{display:'flex',gap:8,alignItems:'center'}}>
                  <div style={{width:22,height:22,borderRadius:5,background:'#E8630A',display:'flex',alignItems:'center',justifyContent:'center',color:'white',fontWeight:700,fontSize:9}}>{c.avatar||'?'}</div>
                  <span style={{color:'#6B6B8A',fontSize:11}}>{c.name||'Admin'}</span>
                </div>
                <span style={{color:'#4A4A6A',fontSize:11}}>{new Date(a.created_at).toLocaleDateString('en-IN',{day:'numeric',month:'short',year:'numeric'})}</span>
              </div>
            </div>
          )
        })}
        {items.length===0 && <div style={{textAlign:'center',padding:48,color:'#4A4A6A'}}><div style={{fontSize:36,marginBottom:12}}>📢</div><div>No announcements yet.</div></div>}
      </div>
  ...
  return (
    <div style={{position:'fixed',inset:0,background:'rgba(0,0,0,0.7)',display:'flex',alignItems:'center',justifyContent:'center',zIndex:1000,padding:20}} onClick={onClose}>
      <div style={{background:'#111120',border:'1px solid #1E1E35',borderRadius:16,padding:28,width:'100%',maxWidth:500}} onClick={e=>e.stopPropagation()}>
        <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:20}}>
          <h3 style={{color:'white',fontFamily:"'Space Grotesk',sans-serif",fontSize:18,fontWeight:700}}>New Announcement</h3>
        ...
        {[['Title *',t,setT,false],['Message *',b,setB,true]].map(([l,v,s,m]) => (
          <div key={String(l)} style={{marginBottom:12}}>
            <label style={{color:'#8888AA',fontSize:10,fontWeight:700,textTransform:'uppercase',letterSpacing:'0.07em',marginBottom:5,display:'block'}}>{l}</label>
        ...
        <div style={{display:'flex',gap:8}}>
          <button onClick={save} disabled={!t||!b||saving} style={{padding:'10px 20px',background:'#E8630A',border:'none',borderRadius:9,color:'white',fontWeight:700,fontSize:13,cursor:'pointer',fontFamily:"'DM Sans',sans-serif"}}>{saving?'Posting…':'Post Announcement'}</button>
