...
export default function BillingClient({ session }: { session: SessionUser }) {
  ...
  return (
    <div>
      <h2 style={{color:'white',fontFamily:"'Space Grotesk',sans-serif",fontSize:20,fontWeight:700,marginBottom:20}}>Billing Dashboard</h2>
      ...
      <div style={{display:'grid',gridTemplateColumns:'repeat(4,1fr)',gap:14,marginBottom:24}}>
        {[['Total Billable',fmt(total),'#EC4899'],['Pending',fmt(totalPending),'#F59E0B'],['Billed',fmt(totalBilled),'#10B981'],['Unpriced',unpriced.length,'#EF4444']].map(([l,v,c]) => (
          <div key={String(l)} style={{background:'#111120',border:'1px solid #1E1E35',borderRadius:12,padding:'18px 20px',borderLeft:`3px solid ${c}`}}>
            ...
            <div style={{color:'white',fontSize:22,fontWeight:700,fontFamily:"'Space Grotesk',sans-serif"}}>{v}</div>
          </div>
        ))}
      </div>
      <div style={{display:'flex',gap:8,marginBottom:16}}>
        {[['all','All'],['unbilled',`Pending (${unbilled.length})`],['billed',`Billed (${billed.length})`]].map(([v,l]) => (
          <button key={String(v)} onClick={()=>setFilter(v)} style={{padding:'7px 14px',background:filter===v?'#EC4899':'#111120',border:filter===v?'none':'1px solid #1E1E35',borderRadius:8,color:filter===v?'white':'#6B6B8A',cursor:'pointer',fontSize:12,fontWeight:600,fontFamily:"'DM Sans',sans-serif"}}>{l}</button>
        ))}
      </div>
      <div style={{background:'#111120',border:'1px solid #1E1E35',borderRadius:14,overflow:'hidden'}}>
        <div style={{display:'grid',gridTemplateColumns:'2.5fr 1fr 1fr 1fr 1fr 1.2fr',padding:'12px 20px',borderBottom:'1px solid #1E1E35',background:'#16162A'}}>
          {['Task','Brand','Status','Amount','Billed On','Actions'].map(h=><div key={h} style={{color:'#6B6B8A',fontSize:11,fontWeight:700,textTransform:'uppercase',letterSpacing:'0.06em'}}>{h}</div>)}
        </div>
        {displayed.map((task:any) => {
          ...
          return (
            <div key={task.id} style={{display:'grid',gridTemplateColumns:'2.5fr 1fr 1fr 1fr 1fr 1.2fr',padding:'13px 20px',borderBottom:'1px solid #1A1A2E',alignItems:'center',background:np?'rgba(239,68,68,0.02)':'transparent'}}>
              <div>
                <div style={{color:'white',fontSize:13,fontWeight:600}}>{task.title}</div>
                <div style={{color:'#6B6B8A',fontSize:11}}>{task.type}</div>
              </div>
              ...
              <div style={{display:'flex',gap:5}}>
                {canEdit && <button onClick={()=>{setPricing(task); setPriceIn(task.billable_amount?.toString()||'')}} style={{padding:'5px 9px',background:'#1A1A2E',border:'1px solid #2A2A45',borderRadius:6,color:np?'#EC4899':'#A0A0C0',cursor:'pointer',fontSize:11,fontWeight:700,fontFamily:"'DM Sans',sans-serif"}}>{np?'₹ Set':'₹ Edit'}</button>}
                {!task.billed_at && canEdit && task.billable_amount && <button onClick={()=>markBilled(task.id)} style={{padding:'5px 9px',background:'#E8630A',border:'none',borderRadius:6,color:'white',cursor:'pointer',fontSize:11,fontWeight:700,fontFamily:"'DM Sans',sans-serif"}}>Bill</button>}
        ...
        {displayed.length===0 && <div style={{padding:32,textAlign:'center',color:'#4A4A6A',fontSize:13}}>No tasks here.</div>}
      </div>
      {pricing && (
        <div style={{position:'fixed',inset:0,background:'rgba(0,0,0,0.7)',display:'flex',alignItems:'center',justifyContent:'center',zIndex:1000,padding:20}} onClick={()=>setPricing(null)}>
          <div style={{background:'#111120',border:'1px solid #1E1E35',borderRadius:16,padding:28,width:'100%',maxWidth:420}} onClick={e=>e.stopPropagation()}>
            <h3 style={{color:'white',fontFamily:"'Space Grotesk',sans-serif",fontSize:17,fontWeight:700,marginBottom:6}}>Set Price</h3>
            <p style={{color:'#6B6B8A',fontSize:13,marginBottom:18}}>{pricing.title}</p>
            <div style={{marginBottom:18}}>
              <label style={{color:'#8888AA',fontSize:10,fontWeight:700,textTransform:'uppercase',letterSpacing:'0.07em',marginBottom:5,display:'block'}}>Amount (₹)</label>
              <input type="number" value={priceIn} onChange={e=>setPriceIn(e.target.value)} placeholder="e.g. 15000" style={{width:'100%',padding:'10px 14px',background:'#1A1A2E',border:'1px solid #EC489950',borderRadius:9,color:'white',fontSize:14,outline:'none',fontFamily:"'DM Sans',sans-serif"}} />
            </div>
            <div style={{display:'flex',gap:8}}>
              <button onClick={setPrice} disabled={!priceIn||saving} style={{padding:'10px 20px',background:'#E8630A',border:'none',borderRadius:9,color:'white',fontWeight:700,fontSize:13,cursor:'pointer',fontFamily:"'DM Sans',sans-serif"}}>{saving?'Saving…':'Save Price'}</button>
              <button onClick={()=>setPricing(null)} style={{padding:'10px 20px',background:'#1A1A2E',border:'1px solid #2A2A45',borderRadius:9,color:'#A0A0C0',fontWeight:600,fontSize:13,cursor:'pointer',fontFamily:"'DM Sans',sans-serif"}}>Cancel</button>
            </div>
          </div>
        </div>
      )}
