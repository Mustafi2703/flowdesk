    ...
    <div>
      ...
      {tab === 'identity' && (
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16 }}>
          <div style={{ background: 'var(--sf-surface)', border: '1px solid var(--sf-border)', borderRadius: 12, padding: 18 }}>
            ...
            {[['Name', brand.name], ['Client Type', brand.client_type], ['Priority', brand.priority], ['Team', `${brand.assigned_members?.length || 0} members`]].map(([l, v]) => (
          ...
          <div style={{ background: 'var(--sf-surface)', border: '1px solid var(--sf-border)', borderRadius: 12, padding: 18 }}>
            ...
            {['Brand Fonts', 'Logo Variants', 'Brand Colors', 'Product Images', 'Photography Style', 'Brand Voice'].map(item => (
              <div key={item} style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '7px 0', borderBottom: '1px solid var(--sf-border)' }}>
                <span style={{ width: 14, height: 14, border: '1px solid var(--sf-border-strong)', borderRadius: 3, display: 'inline-block' }} />
                <span style={{ color: 'var(--sf-muted-2)', fontSize: 12 }}>{item}</span>
              </div>
            ))}
          </div>
        </div>
      ...
      {tab === 'journey' && (
        <div style={{ textAlign: 'center', padding: '40px 20px' }}>
          ...
          <div style={{ color: 'var(--sf-text-secondary)', fontWeight: 600, fontSize: 16, marginBottom: 8 }}>Brand journey — coming next</div>
          <div style={{ color: 'var(--sf-muted-2)', fontSize: 13, maxWidth: 440, margin: '0 auto' }}>Campaign history, milestones, and monthly calendars will live here.</div>
        </div>
      )}
