...
export default function TeamClient({ session }: { session: SessionUser }) {
  ...
  const [panel, setPanel] = useState(role === 'owner' ? 'departments' : 'members')
  ...
  return (
    <PageShell className="sf-team-layout" style={{ padding: 0, height: '100%', gap: 0 }}>
      <div style={{ display: 'flex', height: '100%', minHeight: 0, overflow: 'hidden' }}>
        {/* Left panel — Team / Departments */}
        <aside style={{
          width: 220,
          flexShrink: 0,
          ...
          <div style={{ padding: '16px 14px 12px', borderBottom: '1px solid var(--sf-border)' }}>
            <div style={{ color: 'var(--sf-text)', fontWeight: 700, fontSize: 14 }}>Team</div>
            <div style={{ color: 'var(--sf-muted)', fontSize: 11, marginTop: 4 }}>
              {team.filter(u => u.is_active).length} members · {online} online
            </div>
          </div>
          <div style={{ padding: '10px 10px 14px' }}>
            {TEAM_PANELS.filter(p => p.id !== 'departments' || canViewDepartments).map(p => {
              const count = p.id === 'departments' ? sortedDepartments.length : team.filter(u => u.is_active).length
              const active = panel === p.id
              return (
                <button
                  key={p.id}
                  type="button"
                  onClick={() => { setPanel(p.id); setShowAdd(false); setShowDept(false) }}
                  style={{
                    width: '100%',
                    textAlign: 'left',
                    padding: '10px 12px',
                ...
                >
                  ...
                  <span style={{
                    ...
                    padding: '2px 7px',
                    borderRadius: 999,
                    background: active ? 'rgba(255,255,255,0.2)' : 'var(--sf-surface-2)',
                    color: active ? '#fff' : 'var(--sf-muted)',
                  }}>
                    {count}
                  </span>
                </button>
          ...
          {canManageDepartments && (
            <div style={{ marginTop: 'auto', padding: '12px 10px', borderTop: '1px solid var(--sf-border)' }}>
              <button
                type="button"
                onClick={() => { setPanel('departments'); setShowDept(true); setShowAdd(false); setNotice(''); setError('') }}
        ...
        <div style={{ flex: 1, minWidth: 0, overflowY: 'auto', padding: '20px 24px' }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 16, flexWrap: 'wrap', gap: 10 }}>
            <PageHeader
              title={panel === 'departments' ? 'Departments' : 'Team members'}
              subtitle={panel === 'departments' ? 'Org units with assigned managers' : 'Workload and status per person'}
            />
            {panel === 'members' && canOnboard && (
              <button type="button" onClick={() => { setShowAdd(true); setShowDept(false); setNotice(''); setError('') }} className="sf-btn sf-btn-primary" style={{ marginTop: 4 }}>
                Add user
              </button>
            )}
            {panel === 'departments' && canManageDepartments && (
              <button type="button" onClick={() => { setShowDept(true); setShowAdd(false); setNotice(''); setError('') }} className="sf-btn sf-btn-primary" style={{ marginTop: 4 }}>
                Add department
              </button>
            )}
          </div>
          ...
          {panel === 'members' && (
            <>
              ...
              <Section title="All members" subtitle="Click cards for workload details" flex={1} style={{ marginTop: 16 }}>
